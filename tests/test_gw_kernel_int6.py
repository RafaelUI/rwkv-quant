"""Численная сверка GwQuantLinear (GEMV sb6 + GEMM-путь) для int6
(xbits=2, вторая битплоскость qh2) с референсом x @ dequant(qt).T в fp32,
на живых тензорах REDUCTION v2 (presets.REDUCTION, bits=6 для
proj/cmix/emb_head). Аналог test_gw_kernel.py, но покрывает три разных
group_scale_mode при bits=6:
  proj      -> mode="asym_sb6"      (без AW, без search)  -- xbits=2, search=False
  cmix      -> mode="asym_sb6_aw"   (с AW, с search)       -- xbits=2, search=True
  emb_head  -> mode="asym_sb6_aw"   (с AW, с search)       -- xbits=2, search=True
"""
import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
import torch, numpy as np, mlx.core as mx
from rwkv_quant.formats.writer import quantize_tensor
from rwkv_quant.formats.reader import _dequantize_one
from rwkv_quant.backends.metal.quant_linear_gw import GwQuantLinear
from rwkv_quant.presets import REDUCTION

CKPT = os.path.expanduser("~/Develop/WKV-kvant/rwkv7-g1h-1.5b-ctx10240.pth")
sd = torch.load(CKPT, map_location="cpu")
torch.manual_seed(0)

CASES = [
    ("blocks.0.att.receptance.weight", "proj"),      # asym_sb6, no AW, IN=2048
    ("blocks.0.ffn.key.weight",        "cmix"),       # asym_sb6_aw, IN=2048
    ("blocks.0.ffn.value.weight",      "cmix"),       # asym_sb6_aw, IN=8192
    ("head.weight",                    "emb_head"),   # asym_sb6_aw, IN=2048
]

for key, group in CASES:
    qt = quantize_tensor(key, sd[key], REDUCTION, real_gw=True)
    assert qt.bits == 6, (key, qt.bits)
    lin = GwQuantLinear(qt)
    assert lin.xbits == 2, (key, lin.xbits)
    assert qt.gw_qh is not None and qt.gw_qh2 is not None, key
    ref_w = _dequantize_one(qt).float().numpy()
    for N in (1, 3, 16):
        x = torch.randn(N, qt.shape[1]).numpy().astype(np.float32)
        y = np.array(lin(mx.array(x)))
        ref = x @ ref_w.T
        rel = np.abs(y - ref).max() / (np.abs(ref).max() + 1e-9)
        path = "GEMM" if N >= 16 else "GEMV"
        print(f"{key:34s} grp={group:9s} N={N:2d} {path} relmax={rel:.3e}")
        assert rel < 3e-3, (key, N, rel)
print("INT6 KERNEL NUMERICS OK")
