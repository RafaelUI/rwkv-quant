...
#
# ПРО LoRA -- ЗДЕСЬ ФОРМУЛА УЖЕ ОДИН РАЗ УСТАРЕЛА МОЛЧА. До 16.08 обе
# модели считались с плотными fp16-ветками (100.1 МБ), и это было верно
# ровно до того дня, когда мы их квантовали. На самом деле:
#   у НАС (с 16.08) down при 8 битах gs=64 (8.5 бит/вес) и up при
#     8 битах gs=32 (9.0) -- итого ~55.1 МБ;
#   у НИХ down при 6 битах gs=64 (6.5 бит/вес), а up НЕ КВАНТОВАН вовсе
#     (`lora.2` в их файле -- float16 [2048, 96]) -- итого ~70.9 МБ.
# Проверено чтением их safetensors, а не выведено из конфига.
# Мораль общая: константа, описывающая ЧУЖОЙ артефакт и СВОЙ код разом,
# протухает от правки в любом из двух.
_QW_LAYERS, _QW_HEAD = 1208.0e6, 134.2e6
_LORA_W = 2 * (96 + 96 + 64 + 256) * 2048 * 24 / 2      # весов в down и в up
_LORA_OURS = _LORA_W * (8.5 + 9.0) / 8 / 1e6            # квантованы обе
_LORA_MOLLY = _LORA_W * (6.5 + 16.0) / 8 / 1e6          # up плотный fp16


def traffic_mb(bits_body, bits_head, lora_mb):
...
def main():
    ...
    traf = {"наша (sym+head@8)": traffic_mb(6.5625, 8.5625, _LORA_OURS),
            "штатный mlx-affine": traffic_mb(6.5, 6.5, _LORA_MOLLY)}
