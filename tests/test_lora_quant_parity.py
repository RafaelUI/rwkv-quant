...
def main():
    ...
    ok = True
    # Утверждения 1-4 проверяют САМУ РАСКЛАДКУ, поэтому квантованные ветки
    # должны работать и на префилле: иначе гейт сверял бы fp16 с fp16 и
    # был бы зелёным всегда. Умолчание LORA_Q_DECODE_ONLY=True проверяется
    # отдельно, утверждением 5.
    ...
    # [5] умолчание: квантование ТОЛЬКО на декоде. Префилл обязан быть
    # БИТ-В-БИТ равен плотному (иначе диспетчеризация по T не работает),
    # а декод обязан отличаться (иначе она выключила квантование совсем).
    qm.LORA_Q, qm.LORA_QBITS = "sep", 8
    qm.LORA_Q_DECODE_ONLY = True
    qm.reset_lora_q(model)
    got = {f: run(model, f) for f in (False, True)}
    # Порог у двух путей РАЗНЫЙ, и это не поблажка. При FUSE=False плотная
    # ветка -- те же четыре матмула, что и в базе, поэтому требуется
    # РАВЕНСТВО. При FUSE=True база считает LoRA БАТЧЕНЫМИ стеками (три
    # матрицы с паддингом ранга v 64 -> 96), а под LORA_Q стеки не
    # строятся вовсе и префилл идёт поветочно: порядок суммирования
    # другой, бит-в-бит не обязан. Это и есть та правка, которая
    # ускорила фьюзнутый префилл с 757.8 до 748.4 мс и сняла 57.8 МБ
    # стеков; порог 3e-4 -- уровень документированного расхождения самого
    # фьюза (1.77-1.84e-4).
    # ПОЧЕМУ ПОРОГ ПРИ FUSE=True НЕ НОЛЬ И ПОЧЕМУ ОН ТАКОЙ. На префилле
    # квантования тут нет ВООБЩЕ (LORA_Q_DECODE_ONLY уводит T>1 в плотную
    # ветку), поэтому сравниваются две плотные fp16-реализации одной
    # суммы: батченые стеки фьюза (три матрицы с паддингом ранга) против
    # поветочных матмулов. Это переассоциация, и её величина растёт с
    # глубиной стека: 2.24e-4 на 1.5B (24 слоя) против 6.23e-4 на 2.9B
    # (32 слоя). Порог 1.5e-3 -- это полтора fp16-эпсилона (9.8e-4), то
    # есть заведомо ниже любой содержательной ошибки и заведомо выше
    # шума перестановки. Первая редакция порога (3e-4) была снята с
    # ОДНОГО масштаба и на 2.9B покраснела -- закон 10 применительно к
    # порогам гейта, а не к пресетам.
    # Второе условие важнее первого: greedy обязан совпасть целиком.
    for f, tol in ((False, 0.0), (True, 1.5e-3)):
        rp = relmax(got[f][0], base[f][0])
        rd = relmax(got[f][1], base[f][1])
        same = int((got[f][2] == base[f][2]).sum())
        good = rp <= tol and rd > 0 and same == NGEN
        print(f"[5] LORA_Q_DECODE_ONLY, FUSE={f}: префилл rel {rp:.2e} "
              f"(порог {tol:.0e}), декод rel {rd:.2e} (обязан быть > 0), "
              f"greedy {same}/{NGEN} {'OK' if good else 'КРАСНЫЙ'}")
        ok &= good
