...
"""
    ...
    hb_dec = """
            ushort hs = *(device const ushort*)((device const uchar*)(qb + 4) + off2);
            l0 |= as_type<uchar4>((((uint)(hs >>  0) & 0xFu) * 0x00204081u & 0x01010101u) << 4);
            l1 |= as_type<uchar4>((((uint)(hs >>  4) & 0xFu) * 0x00204081u & 0x01010101u) << 4);
            h0 |= as_type<uchar4>((((uint)(hs >>  8) & 0xFu) * 0x00204081u & 0x01010101u) << 4);
            h1 |= as_type<uchar4>((((uint)(hs >> 12)       ) * 0x00204081u & 0x01010101u) << 4);
""" if xbits >= 1 else ""
    hb2_dec = """
            ushort hs2 = *(device const ushort*)((device const uchar*)(qb + 5) + off2);
            l0 |= as_type<uchar4>((((uint)(hs2 >>  0) & 0xFu) * 0x00204081u & 0x01010101u) << 5);
            l1 |= as_type<uchar4>((((uint)(hs2 >>  4) & 0xFu) * 0x00204081u & 0x01010101u) << 5);
            h0 |= as_type<uchar4>((((uint)(hs2 >>  8) & 0xFu) * 0x00204081u & 0x01010101u) << 5);
            h1 |= as_type<uchar4>((((uint)(hs2 >> 12)       ) * 0x00204081u & 0x01010101u) << 5);
""" if xbits >= 2 else ""
    body = """
...
"""
    kern = mx.fast.metal_kernel(
        name=f"k3f_gw{4 + xbits}_s{NSG}r{RS}_{IN}_{OUT}",
...
class Syn:
    def __init__(self, OUT, IN, xbits, rng):
        ...
        self.qblkF = mx.array(np.ascontiguousarray(
        ...
        mx.eval(self.qblk, self.qblkF, self.qsqm, self.ddm)
    ...
    def f(self, x, xbsum, NSG, RS):
        ...
        n_tg = self.OUT // (NSG * RS)
        return kern(
            inputs=[x, self.qblkF, self.qsqm, self.ddm, xbsum],
            grid=(n_tg * NSG * 32, 1, 1), threadgroup=(NSG * 32, 1, 1),
            output_shapes=[(1, self.OUT)], output_dtypes=[mx.float32])[0]
...
print("=== кернель-3 фаза F: полублок (16 значений/поток) ===")
