"""PoC спекулятивного декодинга (greedy) на affine6-модели.
...
print("build affine6...", flush=True)
...
while len(out_spec) < N_GEN:
    if len(pending) > FLUSH:
        lg, st = model.forward_stateful(mx.array([pending]), st, last_only=True)
    ...
    p = len(pending)
    x = pending + draft
    lg, st_new = model.forward_stateful(mx.array([x]), st, last_only=False)
