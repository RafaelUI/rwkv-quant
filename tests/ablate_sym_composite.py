...
def base_cfg():
    """REDUCTION со статистикой ЭТОГО чекпоинта.

    act_stats_path в пресете зашит на /tmp/act_stats_1p5b.pt, то есть
    результат зависит от содержимого /tmp (докстринг presets.py про это
    честно предупреждает). Здесь путь задаётся явно всегда.
    """
