...
# кеш, своп неподвижен на всех прогонах):
#
#           резидентно   footprint   декод
#     1.5B    -123 МБ     -537 МБ    +0.5%
#     2.9B    -156 МБ     -606 МБ    +0.2%
#
# Цена на обоих масштабах лежит ВНУТРИ разброса плеча (0.7-2.6%), а приз
# с масштабом РАСТЁТ. Выход бит-в-бит (test_emb_gather_parity), поэтому
# смена умолчания не двигает ни одного числа качества -- это чистая
# память. COMPRESSION правка не затрагивает: там emb в sb6, применимость
# не проходит и путь остаётся плотным.
#
...
def _layer_norm(x, weight, bias, eps=1e-5, fast=None):
    ...
    по памяти ~0.01. Замер 12.09 (tests/_sess/bench_head_ab.py, чередование
    в ОДНОМ процессе, закон 25) разложил её: снятие mean/var из этой функции
    даёт 0.497 мс, token-shift 0.087, лерпы в остатке ~0.07. То есть статья
    сидит в РЕДУКЦИЯХ рукописной нормы (mean, вычитание, квадрат, второй
    mean -- несколько проходов), а не в цене запусков и не в лерпах.
    Аффинный хвост бесплатен: плечи x*w+b и passthrough неразличимы.

    ЧТО БЕРЁТ ПРИМИТИВ (bench_head_fast_ab.py, bench_lnfast_scale.py):
        0.1B  декод  2.911 -> 2.708 мс  (+7.0%, 26 вызовов нормы)
        1.5B  декод 13.649 -> 13.218 мс (+3.2%, 50 вызовов; 0.431 мс
                    воспроизведено в двух процессах до третьего знака)
        2.9B  декод 25.065 -> 24.168 мс (+3.6%, 66 вызовов)
        префилл: 1.5B T=512 +1.3%, T=1024 +1.7%; 0.1B +2.8%; 2.9B +1.1%
    На 1.5B это 87% потолка статьи -- поэтому ЯДРО "шапки слоя" писать не
    надо, оно боролось бы за оставшиеся 0.066 мс при цене запуска 11.8 мкс
    на слой (замер 11.09).

    ЧИСЛА МЕНЯЮТСЯ: порядок редукции другой, бит-в-бит не ожидается и не
    требуется. Умолчание False до решения владельца; гейт --
    tests/test_fast_ln_parity.py, пороги взяты те же, что 10.09 для ядер.
    ЛОВУШКА mx.compile: step трассирует ветку на момент компиляции, смена
