...
REDUCTION = QuantConfig(
    ...
    # `emb` -- БЕЗ `_aw`, и это НЕ смена схемы (26.08). Статистики
    # активаций для `emb.weight` не существует ни в одном файле и не может
    # существовать: у таблицы эмбеддингов вход -- индексы, E[x^2] по ним
    # не определено, и `ACT_RECORDER` пишет только по входу линейных
    # слоёв. `get_ex2` возвращал None ТИХО (закон 15 покрывал лишь
    # несовпадение длины, но не отсутствующий ключ), поэтому `sym_aw` на
    # emb всё это время исполнялся как `sym`. Проверено байтами:
    # tests/test_emb_aw_is_noop.py -- два режима дают ОДИН файл, при том
    # что на `head`, где статистика есть, они расходятся.
    # Следствие: НИ ОДНО записанное число не меняется этой правкой.
...
# 09.09: бюджет перераспределён внутри групп. Замерено помотрично, что
# чувствительность внутри группы расходится в разы: r/k несут вчетверо-
# вдевятеро меньше v/o, а cmix.key -- больше всех в модели. Пятый бит на
# proj не окупался (снятие ничего не стоит), а первый бит на cmix.key --
# самая выгодная сделка в модели (1.94 п.п. ppl на 100 МБ). Итог: те же
# 970.5 МБ, ppl +2.850% вместо +3.390%, KL 0.0360 вместо 0.0386
# (парная разность +0.00257, 95% CI [+0.00169; +0.00352]).
