...
# ПОРОГ ВЫБРАН ЗАМЕРОМ, А НЕ ОСТОРОЖНОСТЬЮ. Нарезка бесплатна на CPU
# (2.6 с в обоих случаях), но НЕ на MPS: там каждая полоса -- отдельная
# серия диспатчей. Форма cmix.key 1.5B [8192, 2048] на MPS, AW-поиск:
#   полоса  32 МБ -> 2 полосы -> 2300 мс
#   полоса 128 МБ -> 1 полоса -> 1330 мс
# то есть разбиение НАДВОЕ стоило 1.7x. При 128 МБ большие матрицы
# (cmix, proj) считаются целиком и не платят ничего, а нарезка включается
# там, ради чего и вводилась: emb/head у 2.9B [65536, 2560] дают 5 полос
# и пик около 1.3 ГБ вместо 5.67.
_CHUNK_BYTES = int(os.environ.get("RWKVQ_GW_CHUNK_MB", "128")) << 20
